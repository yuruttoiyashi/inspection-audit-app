...
export default function InspectionTargetsPage() {
  ...
  const handleEdit = (target: InspectionTarget) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (editingId) {
      ...
      if (error) {
        setMessage('点検対象の更新に失敗しました。');
        setMessageType('error');
      } else {
        setMessage('点検対象を更新しました。');
        setMessageType('success');
        resetForm();
        await fetchTargets();
      }
    } else {
      const { error } = await supabase.from('inspection_targets').insert([
        {
          name: formState.name.trim(),
          category: formState.category.trim() || null,
          location: formState.location.trim() || null,
      ...
      if (error) {
        setMessage('点検対象の登録に失敗しました。');
        setMessageType('error');
      } else {
        setMessage('点検対象を登録しました。');
        setMessageType('success');
        resetForm();
        await fetchTargets();
      }
  ...
  const handleToggleActive = async (target: InspectionTarget) => {
  ...
  const filteredTargets = useMemo(() => {
    ...
    return targets.filter((target) => {
      const matchesKeyword =
        normalizedKeyword === '' ||
        [target.name, target.category ?? '', target.location ?? '']
          .join(' ')
          .toLowerCase()
      ...
      const matchesStatus =
        statusFilter === 'all' ||
        (statusFilter === 'active' && target.is_active) ||
        (statusFilter === 'inactive' && !target.is_active);
  ...
  }, [targets, keyword, statusFilter, categoryFilter]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">点検対象管理</h1>
        <p className="mt-1 text-sm text-slate-600">
          点検対象の登録・編集・有効/無効の管理を行います。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-xl font-semibold text-slate-900">
            {editingId ? '点検対象を編集' : '新規登録'}
          </h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">
            {editingId
              ? '選択中の点検対象を更新できます。'
              : 'まずはMVPとして、点検対象の基本情報を登録します。'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              ...
              >
                点検対象名 <span className="text-rose-500">*</span>
              </label>
              <input
                id="name"
                type="text"
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-900">点検対象一覧</h2>
            ...
            <div className="text-sm text-slate-500">
              {loading ? '読み込み中...' : `${filteredTargets.length}件表示中 / 全${targets.length}件`}
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="xl:col-span-2">
              ...
              <input
                ...
                placeholder="点検対象名 / カテゴリ / 設置場所で検索"
